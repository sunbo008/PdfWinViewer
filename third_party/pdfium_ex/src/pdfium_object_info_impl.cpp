...
PdfiumEx_GetPageObjectInfoEx(FPDF_PAGE page, FPDF_PAGEOBJECT page_object) {
    ...
    CPDF_PageObject* pPageObj = GetInternalPageObject(page_object);
    if (!pPage || !pPageObj) return nullptr;
    ...
    if (pdf_obj && pdf_obj->IsReference()) {
        ...
        if (doc) {
            RetainPtr<CPDF_Object> real_obj = doc->GetOrParseIndirectObject(obj_info->obj_num);
            if (real_obj) {
                std::string dict_content = ObjectToPdfString(real_obj.Get());
...
PdfiumEx_GetRawObjectContent(FPDF_DOCUMENT document, uint32_t obj_num, uint32_t gen_num) {
    ...
    // 转换为PDF格式字符串
    std::string content = ObjectToPdfString(obj.Get());
...
PdfiumEx_GetPageObjectDict(FPDF_PAGE page) {
    ...
    // 将页面字典转换为PDF格式
    std::string dict_content = ObjectToPdfString(page_dict);
    obj_info->dict_length = dict_content.length();
    obj_info->raw_dict_content = static_cast<char*>(
        malloc(obj_info->dict_length + 1));
    if (obj_info->raw_dict_content) {
...
// 辅助函数：创建树节点
static PDFIUM_EX_OBJECT_TREE_NODE* CreateTreeNode(uint32_t obj_num, uint32_t gen_num, const char* content, int depth) {
    ...
    if (node->children) {
        memset(node->children, 0, sizeof(PDFIUM_EX_OBJECT_TREE_NODE*) * node->max_children);
    }
...
// 辅助函数：添加子节点
static void AddChildNode(PDFIUM_EX_OBJECT_TREE_NODE* parent, PDFIUM_EX_OBJECT_TREE_NODE* child) {
    if (!parent || !child || !parent->children) return;
    
    if (parent->child_count >= parent->max_children) {
        // 扩展容量
        int new_capacity = parent->max_children * 2;
        auto* new_children = static_cast<PDFIUM_EX_OBJECT_TREE_NODE**>(
            realloc(parent->children, sizeof(PDFIUM_EX_OBJECT_TREE_NODE*) * new_capacity));
...
// 队列式构建对象树（替代递归方式）
static void BuildObjectTreeWithQueue(FPDF_DOCUMENT document, PDFIUM_EX_OBJECT_TREE_NODE* root, int max_depth) {
    ...
    // 维护查找队列和对象树映射
    std::queue<std::pair<uint32_t, PDFIUM_EX_OBJECT_TREE_NODE*>> analysis_queue; // <对象号, 父节点>
    std::unordered_map<uint32_t, PDFIUM_EX_OBJECT_TREE_NODE*> object_tree_map; // 对象号 -> 树节点映射
    ...
    while (!analysis_queue.empty() && processed_count < 10000) { // 限制总处理数量防止无限循环
        ...
        if (!current_node || current_node->depth >= max_depth) continue;
        
        // 获取当前对象
        RetainPtr<const CPDF_Object> obj = pDoc->GetOrParseIndirectObject(current_obj_num);
        ...
        for (const auto& pair : locker) {
            ...
            if (value->IsReference()) {
                ...
                for (size_t i = 0; i < arr->size() && i < 100; ++i) { // 支持大型注释数组
                    const CPDF_Object* arr_obj = arr->GetObjectAt(i);
                    if (arr_obj && arr_obj->IsReference()) {
            ...
            } else if (value->IsDictionary() && current_node->depth < 2) { // 只在前2层处理字典引用
                const CPDF_Dictionary* sub_dict = value->AsDictionary();
                CPDF_DictionaryLocker sub_locker(sub_dict);
                for (const auto& sub_pair : sub_locker) {
                    if (ref_obj_nums.size() >= 100) break; // 限制总引用数量
                    const CPDF_Object* sub_obj = sub_pair.second.Get();
                    if (sub_obj && sub_obj->IsReference()) {
        ...
        for (uint32_t ref_obj_num : ref_obj_nums) {
            ...
            // 限制子节点数量
            if (current_node->child_count >= 100) break;
            
            // 获取引用对象的内容
            char* content_str = PdfiumEx_GetRawObjectContent(document, ref_obj_num, 0);
            ...
            // 创建子节点
            PDFIUM_EX_OBJECT_TREE_NODE* child = CreateTreeNode(ref_obj_num, 0, content_str, current_node->depth + 1);
            if (child) {
...
PdfiumEx_BuildObjectTree(FPDF_DOCUMENT document, FPDF_PAGE page, int max_depth) {
    if (!document || !page) return nullptr;
    
    CPDF_Page* pPage = GetInternalPage(page);
    if (!pPage) return nullptr;
    
    // 限制最大深度为100，允许更完整的分析
    if (max_depth <= 0 || max_depth > 100) max_depth = 100;
    ...
    // 安全检查对象编号
    if (page_obj_num == 0) return nullptr;
    
    std::string page_content = ObjectToPdfString(page_dict);
    if (page_content.empty()) return nullptr;
    
    // 创建根节点（页面对象）
    PDFIUM_EX_OBJECT_TREE_NODE* root = CreateTreeNode(page_obj_num, page_gen_num, page_content.c_str(), 0);
